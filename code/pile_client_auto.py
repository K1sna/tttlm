...
def parse_args():
    ...
    parser.add_argument('--query', type=str, default='One apple a day, keep doctors away.')
    ...
    if os.path.exists(args.query_file):
        with open(args.query_file, 'r', encoding='utf-8') as infile, \
            ...
            for line in infile:
                ...
                if query_text:
                    start_time = time.time()
                    _, results = client.string_query(query_text, args.num_neighbors)
                    logging.info('Query time: %.2f', time.time() - start_time)
                    result_data = {'search': query_text}
                    for i, text in enumerate(results, start=1):
                        result_data[f'result{i}'] = text
